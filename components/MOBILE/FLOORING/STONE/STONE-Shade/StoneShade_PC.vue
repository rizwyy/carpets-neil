    ...
    <transition @beforeEnter="handleDetailsDOMEntry('stone_Finish_Details_PC')">
      ...
      >
        ...
        <div class="h-max w-full flex justify-between text-[3.8vh] z-[9]">
          <span
            class="text-detailsContainer_heading_PC text-left text-balance stone_Finish_Details_PC-HEADING opacity-0 translate-x-[40%]"
        ...
        <div class="h-max w-full flex justify-between gap-[2vw] z-[9]">
          ...
          <div
            :class="[
              'materialItem_PC overflow-hidden rounded-md h-[28vh] stone_Finish_Details_PC-CONTAINER opacity-0 translate-y-[20%] w-full  border-[2px] border-[#f1f1f1]  w flex items-end justify-between text-[#f1f1f1] px-[1.6vw] py-[.8vh] text-[2.4vh] relative cursor-pointer',
              { selected: userStore.preference.spec_4 === 'Light' },
            ]"
            @click="toggleSelect('Light')"
          >
            <div
              v-show="userStore.preference.spec_4 === 'Light'"
            ...
            <NuxtImg
              ...
              alt="#"
            />
            <span class="text-[1.8rem]">Light</span>
          </div>
          <!-- Medium -->
          <div
            :class="[
              'materialItem_PC overflow-hidden rounded-md h-[28vh] stone_Finish_Details_PC-CONTAINER opacity-0 translate-y-[20%] w-full  border-[2px] border-[#f1f1f1]  w flex items-end justify-between text-[#f1f1f1] px-[1.6vw] py-[.8vh] text-[2.4vh] relative cursor-pointer',
              { selected: userStore.preference.spec_4 === 'Medium' },
            ]"
            @click="toggleSelect('Medium')"
          >
            <div
              v-show="userStore.preference.spec_4 === 'Medium'"
              class="selectedDiv h-full w-full from-[20%] absolute bottom-0 left-0 bg-opacity-[.3] z-[1] flex items-start justify-end px-[1vw] py-[1vh]"
            >
              <!-- DESKTOP -->
              <!-- Selected indicator without text -->
            ...
            <NuxtImg
              ...
              alt="#"
            />
            <span class="text-[1.8rem]">Medium</span>
          </div>
          <!-- Dark -->
          <div
            :class="[
              'materialItem_PC overflow-hidden rounded-md h-[28vh] stone_Finish_Details_PC-CONTAINER opacity-0 translate-y-[20%] w-full  border-[2px] border-[#f1f1f1]  w flex items-end justify-between text-[#f1f1f1] px-[1.6vw] py-[.8vh] text-[2.4vh] relative cursor-pointer',
              { selected: userStore.preference.spec_4 === 'Dark' },
            ]"
            @click="toggleSelect('Dark')"
          >
            <div
              v-show="userStore.preference.spec_4 === 'Dark'"
              class="selectedDiv h-full w-full from-[20%] absolute bottom-0 left-0 bg-opacity-[.3] z-[1] flex items-start justify-end px-[1vw] py-[1vh]"
            >
              <!-- DESKTOP -->
              <!-- Selected indicator without text -->
            ...
            <NuxtImg
              ...
              alt="#"
            />
            <span class="text-[1.8rem]">Dark</span>
          </div>
          <!-- DESKTOP -->
        </div>
      </div>
...
.selected {
  ...
  border: 4px solid #333; /* Adjust border color as needed */
}
</style>
