      ...
      <div class="h-max w-full flex justify-between gap-[2vw] z-[9]">
        ...
        <div
          :class="[
            ...
            { selected: spec_1.includes('Tiles') },
          ]"
          @click="toggleSelect('Tiles')"
        >
          <div
            v-show="spec_1.includes('Tiles')"
          ...
          <NuxtImg
            ...
            src="/stone/stoneTiles.webp"
            alt=""
          />
          <span class="text-[3.4vh]">Tiles</span>
        </div>
        ...
        <div
          :class="[
            'h-[28vh] w-full rounded-md  border-[2px] border-[#f1f1f1]  w flex items-end justify-between text-[#f1f1f1] px-[1.6vw] py-[.8vh] text-[2.4vh] relative cursor-pointer',
            { selected: spec_1.includes('Slabs') },
          ]"
          @click="toggleSelect('Slabs')"
        >
          <div
            v-show="spec_1.includes('Slabs')"
            class="selectedDiv h-full w-full from-[20%] absolute bottom-0 left-0 bg-opacity-[.3] z-[1] flex items-start justify-end px-[1vw] py-[1vh]"
          >
            <!-- Selected indicator without text -->
            <span
          ...
          <NuxtImg
            ...
            loading="eager"
            class="h-full w-full object-cover absolute left-0 bottom-0 z-[-9]"
            src="/stone/stoneSlabs.webp"
            alt="Slabs"
          />
          <span class="text-[3.4vh]">Slabs</span>
        </div>
      </div>
    </div>
  </div>
...
const toggleSelect = (category) => {
  if (spec_1.value === category) {
    ...
    scrollToEl("stoneMaterialContainerPC");
