<template>
  <transition @beforeEnter="handleDetailsDOMEntry('RevForm_PC')">
    <div
      v-show="
    ...
    >
      ...
      >
        ...
        <div class="h-max w-full flex flex-col gap-[2vh]">
          ...
          <!--  -->
          <div
            class="h-max w-full border-[#999] flex flex-col gap-[8vh] mb-[4vh] px-[2vw]"
          >
            <span
              @click="getHistory"
              class="text-[2.4vh] tracking-[-.05vw] w-full text-detailsContainer_heading_PC border-t-[1px] px-[2vw] pt-[4vh]"
              >Add On's</span
            >
            <div class="h-max w-full flex">
              <div
                class="h-max w-full flex flex-col gap-[2vh] text-[#777] px-[2vw]"
              >
        ...
        <div class="h-max w-full flex px-[2vw]">
          <div
            class="h-max w-[50%] flex flex-col justify-center items-start gap-[4vh]"
          >
            <span
              class="text-left text-[#777] text-[1.8vh] border-[#999] w-[80%]"
              ><ul class="font-[500]">
                Note:
              </ul>
            ...
            <button
              @click="() => HandleAddMore()"
              class="bg-[#DCE9FE] text-center revCard-HEADING active:scale-[.93] flex items-center justify-center backdrop-blur-[8px] w-[88%] border-[1.8px] tracking-[.2vw] border-[#333] rounded-md py-[5vh] uppercase font-[400] text-[2vh] px-[4vw] outline-none focus:border-black text-[#333]"
            >
          ...
          <div
            class="h-full w-[50%] flex flex-col justify-center items-start gap-[4vh]"
          >
            <span
              class="text-left text-[#777] text-[1.8vh] border-[#999] w-[100%]"
              ><ul class="font-[500]">
                Note:
              </ul>
              By clicking the button below, your order will be confirmed using
              your
              {{
                userStore.preference.orderMethod === "whatsapp"
                  ? "phone number"
                  : "email"
              }}. Once confirmed, you won't be able to make further changes, but
              you can still explore other flooring selections.</span
            >
            <button
              @click="HandleOrderConfirmation"
              class="bg-[#fff9] text-center revCard-HEADING active:scale-[.93] backdrop-blur-[8px] w-full border-[3.2px] tracking-[.2vw] border-[#999] rounded-md py-[4.4vh] uppercase font-[400] text-[2.4vh] px-[2vw] outline-none focus:border-black flex justify-center items-center"
            >
...
import PreferenceCardPC from "./PreferenceCardPC.vue";
import UploadContainerPC from "./UploadContainerPC.vue";
import ReusablePreferenceCardPC from "./ReusablePreferenceCardPC.vue";
