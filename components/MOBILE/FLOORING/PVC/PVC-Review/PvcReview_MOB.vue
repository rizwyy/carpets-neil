<template>
  <transition @beforeEnter="handleDetailsDOMEntry('pvcRevForm')">
    <div
      v-show="
    ...
    >
      ...
      >
        <div class="h-max w-full flex flex-col items-start gap-[1.2vh]">
          ...
          <button
            @click="scrollToTop"
            class="pvcRevForm-HEADING opacity-0 underline underline-offset-[.5vh] translate-y-[20%] text-[2vh] text-[#999] font-[500] w-max flex gap-[.6vw] items-center z"
          >
            Edit Details
            <svg
        ...
        <span>
          <span
            class="mt-[1vh] w-full border-[2px] pvcRevForm-HEADING translate-y-[20%] opacity-0 rounded-full px-[4vw] py-[1vh] border-[#666] bg-[#fff7]"
        ...
        <div class="h-max w-full flex justify-between items-center">
          <span>
            <span
              class="pvcRevForm-HEADING translate-y-[20%] opacity-0 w-full border-[2px] rounded-full px-[4vw] py-[1vh] border-[#666] bg-[#fff7]"
              ><span>Finish:&nbsp;</span>{{ userStore.preference.spec_4 }}</span
            >
          </span>
          <span
            class="pvcRevForm-HEADING translate-y-[20%] opacity-0 w-max border-[2px] rounded-full px-[4vw] py-[1vh] border-[#666] bg-[#fff7]"
            >Budget: {{ userStore.preference.budget }}</span
          >
        </div>
        <div
          class="pvcRevForm-HEADING translate-y-[20%] opacity-0 h-max w-max flex flex-col gap-[1.8vh]"
        >
        ...
        <a
          href="/flooring/pvc/success"
          @click="handleClick"
          class="bg-[#fff9] text-center pvcRevForm-HEADING active:scale-[.93] backdrop-blur-[8px] w-[88vw] border-[3.2px] tracking-[.2vw] border-[#333] rounded-md py-[2.4vh] uppercase font-[400] text-[2.4vh] px-[2vw] outline-none focus:border-black"
        >
          CONFIRM
        </a>
      </div>
...
const handleClick = () => {
  ...
  useFetch("/api/set-cookie")
    ...
    .then((logData) => {
      ...
      scrollToBottom();
      handleLoadingExit();
      router.push("/flooring/pvc/success");
      console.log("SUCCESS");
